...
# In this regular expression, we use "^[0-9]+[a-zA-Z #]+" to ensure the address begins with a number plus street name.
# (e.g., "74 W College St")
# Then we use "[0-9]*[a-zA-Z #]+" to address the situation where the street name contains a number.
# (e.g., "30 W 80th St #1995", the number 80 is not apartment number).
# Finally, we use "([0-9]+).*$" to extract the apartment number, 
# the apartment number is stored in a temporary variable and can be access through "\\1".
apartment_numbers_str <- gsub("^[0-9]+[a-zA-Z #]+[0-9]*[a-zA-Z #]+([0-9]+).*$",
                              "\\1",
...
As the results shown, for digits 1, 2, 5, 7, 10, there is significant divergence between samples' density and Benford's density. And in the chi-squared test, p-value is very small (i.e., $3.175664e-05$), which demonstrates that we can reject the null hypothesis, and, in other words, the leading digits of apartment numbers don't follow Benford's law. Therefore, I argue that apartment numbers would not pass as real data. 
